...
public class DepthFirstOrder {
    ...
    public DepthFirstOrder(Digraph G) {
        pre    = new int[G.getVertexNum()];
        post   = new int[G.getVertexNum()];
    ...
    public DepthFirstOrder(EdgeWeightedDigraph G) {
        pre    = new int[G.getVertextNum()];
        post   = new int[G.getVertextNum()];
        postorder = new Queue<Integer>();
        preorder  = new Queue<Integer>();
        marked    = new boolean[G.getVertextNum()];
        for (int v = 0; v < G.getVertextNum(); v++)
            if (!marked[v]) dfs(G, v);
    }
